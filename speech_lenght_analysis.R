...
lenghts_list <- lapply(ctm_dat_list, function(x){
  return(list("wordno" = nrow(x),  "mean" = mean(x$length), "number" = length(which(x$length > 1.25))))
})

cond <- sapply(lenghts_list, function(x) x$wordno < 50 )
cond_list <- names(which(cond))
cat(cond_list, sep='\n')

bad_list <- sapply(cond_list, function(cnd_name) {
  ind = which(names(lenghts_list) == cnd_name)
  lenghts_list[[ind]]$wordno
})

blDF <- as.data.frame(bad_list)

blDF[order(blDF[,"bad_list"], decreasing = F), , drop = FALSE]
# za strona https://stackoverflow.com/questions/16695070/sorting-a-data-frame-in-r-maintaining-row-names